...
describe('SessionPersistence service registration', () => {
  ...
  it('rejects an append after the durable cursor advanced behind its back (concurrent writer)', async () => {
    const id = SessionId('concurrent-writer')
    const store: MemoryStore = new Map()
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    const session = ctx.sessions.create(id, { meta: { cwd: '/w' } })
    const fiber = await ctx.plugin(MemoryPersistence, { store })
    ...
    try {
      ...
      // A second writer on the shared store appends a resume boundary without
      // this coordinator's knowledge; the in-memory cursor is now stale.
      const entry = store.get(id)!
      entry.events.push({ type: 'session/end-seed', seq: entry.events.length, time: 7, data: {} })

      // The live session mints its next seq from its own log, so the contiguity
      // check alone cannot see the collision; the revision guard must refuse it.
      session.append('turn/start', { turn: 2 })
      await expect(ctx.sessions.flush(session)).rejects.toThrow(/changed on disk since its last sync/)
      expect(store.get(id)!.events.length).toBe(oneTurnLog().length + 1)
    } finally {
      await Promise.allSettled([fiber.dispose()])
    }
  })
