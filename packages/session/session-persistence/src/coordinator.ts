...
interface SessionState {
  ...
  cursor: number
  /**
   * The durable artifact revision the `cursor` was last synced against. An
   * append re-reads the live revision and refuses a mismatch so a concurrent
   * writer (or a mismatched build) cannot silently duplicate sequence numbers.
   */
...
export class PersistenceCoordinator<TornMarker = unknown> {
  ...
  private async appendCore(id: SessionId, events: readonly SessionEvent[]): Promise<void> {
    ...
    // Concurrent-writer guard: the in-memory cursor is only authoritative while
    // the durable artifact revision is unchanged. A second writer (another
    // process, or a mismatched build sharing the same session root) that
    // appended since our last sync would otherwise duplicate sequence numbers
    // silently. Materialization is already exclusive (link/EEXIST), so only the
    // append path needs the re-check.
    if (state.materialized) {
      const durable = await this.backend.readStoredRevision(id)
      if (durable !== state.revision) {
        throw new Error(`session "${id}" changed on disk since its last sync (stale cursor ${state.cursor}); reload it before appending`)
      }
    }
    ...
    state.revision = await this.backend.readStoredRevision(id)
  ...
  private async adoptLivePrefix(session: Session, seed: readonly SessionEvent[], stored: StoredPrefix<TornMarker>): Promise<void> {
    ...
    // Re-read after any repair so the recorded revision matches the committed bytes.
    const revision = await this.backend.readStoredRevision(session.header.id)
